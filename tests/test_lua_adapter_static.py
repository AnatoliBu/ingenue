...
class LuaAdapterStaticTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        path = Path(__file__).resolve().parents[1] / "web" / "lib" / "mod.lua"
    ...
    def test_applied_ack_and_state_paths_are_present(self):
        for path in (
            ...
            "/ingenue/ack",
            "/ingenue/reject",
            "/ingenue/script/state",
            "/ingenue/grid/frame",
        ):
            self.assertIn(path, self.source)

    def test_state_port_supports_flat_and_legacy_nested_install_layouts(self):
        self.assertIn("ingenue/data/realtime-state-port", self.source)
        self.assertIn("ingenue/web/data/realtime-state-port", self.source)
