...
export async function mountInspector(root=document) {
  ...
  root.querySelectorAll('[data-key]').forEach(button=>{const n=Number(button.dataset.key);const down=()=>session.command({target:'control',action:'key',args:{n,z:1}});const up=()=>session.command({target:'control',action:'key',args:{n,z:0}});button.addEventListener('pointerdown',down);button.addEventListener('pointerup',up);button.addEventListener('pointercancel',up);button.addEventListener('pointerleave',event=>{if(event.buttons)up();});});
