...
try:
    ...
    from .realtime_gamepad import GamepadAppliedAdapter, GamepadAppliedHub
...
MidiAppliedAdapter = GamepadAppliedAdapter
MidiAppliedHub = GamepadAppliedHub
...
def serve_realtime(host, port, legacy):
    ...
    try:
        with OriginCheckedServer((host, port), hub, http_port, allowed) as server:
            print("ingenue realtime on {}:{}/realtime (Lua-applied, Grid/Arc/Gamepad/MIDI-ready, origin-checked)".format(host, port), flush=True)
