...
export async function runTransportDiagnostics(options={}){
  ...
  onProgress('done');
  return {benchmark_version:3,started_at:new Date().toISOString(),page:typeof location==='undefined'?null:location.href,user_agent:typeof navigator==='undefined'?null:navigator.userAgent,online:typeof navigator==='undefined'?null:navigator.onLine,config:{samples,concurrency,httpUrl,wsUrl},http_serial:{latency:summarize(serialHttp.values),errors:serialHttp.errors},http_concurrent:{latency:summarize(concurrentHttp.values),errors:concurrentHttp.errors},websocket_connect:{latency:summarize(wsConnect.values),errors:wsConnect.errors},matron_roundtrip:{latency:summarize(matronRoundTrip.values),errors:matronRoundTrip.errors},limits:['HTTP results stop at the Ingenue Python response.','Matron round-trip measures browser → matron Lua print → browser, not the OSC control path.','This benchmark does not measure audio latency.']};
}
