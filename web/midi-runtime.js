...
class LatestValueLane {
  constructor(send, register) {
    this.send=send;this.register=register;this.inflight=null;this.queued=null;
  }
  push(value){if(this.inflight){this.queued=value;return null;}return this.#dispatch(value);}
  settle(id,status){if(!this.inflight||this.inflight.id!==id)return null;const completed=this.inflight;this.inflight=null;const desired=this.queued;this.queued=null;if(status==='uncertain')return this.#dispatch(desired??completed.value);if(status!=='ack'||desired==null)return null;return this.#dispatch(desired);}
...
export class MidiRuntime {
  ...
  async activate(mappings) {
    ...
    try {
      ...
      if(mapping.target.kind==='param'&&mapping.mode==='absolute'){
        ...
        state.lane=new LatestValueLane(value=>this.send({target:'param',action:'set_normalized',args:{id:mapping.target.id,value}}),
          (id,lane)=>this.commandOwners.set(id,{state,lane}));
      }
  ...
  settle(detail){
    ...
    if(descriptor){owner.state.descriptor=descriptor;owner.state.pickup?.updateApplied(Number(descriptor.normalized));}
